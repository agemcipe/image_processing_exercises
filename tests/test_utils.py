...
@pytest.mark.parametrize("method", ["loop", "mask"])
def test_erosion(size, expected_result, method):

    actual_result = utils.erosion(
        utils.get_image("03ab", "immed_gray_inv"), size, method
    )
...
@pytest.mark.parametrize("method", ["loop", "mask"])
def test_dilation(size, expected_result, method):

    assert utils.images_are_equal(
        utils.dilation(utils.get_image("03ab", "immed_gray_inv"), size, method),
        expected_result,
    )
